...
class DLL:
    ...
    def push(self, val):
        ...
        if self.head != None: # NINJA NOTES : else if
            new_node.prev = None
    ...
    def delete(self, node):
        ...
        if node.prev != None and node.next != None:
            ...
            self.length -= 1
            return # get rid of the three returns and return at the end
    ...
    def index(self, i):
        ...
        while i < self.length:
            if i == idx:
                return current.val # change to current
            else:
                idx += 1
                current = current.next
        # add check in cases i = -1 or a big number aka i doesnt fall into the bounds of the list
...
def test_multiply_all_pairs():
    ...
    dll_new.push(1)
    dll_new.push(2)
    dll_new.push(3) #investigate why??? data being overwritten
